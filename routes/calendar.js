...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const formattedStart = adjustedStart
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");
    const formattedEnd = adjustedEnd
      .toISOString()
      .slice(0, 19)
      .replace("T", " ");

    // Insert the new event into the database
    await db(`INSERT INTO calendar (title, start, end) VALUES (?, ?, ?)`, [
      title,
      formattedStart,
      formattedEnd,
    ]);
...
router.put("/:id", async (req, res) => {
  ...
  if (!title && !start && !end) {
    return res
      ...
      .json({
        message:
          "At least one field (title, start, end) is required for updating.",
      });
  }
