...
router.get(
  "/subjects/:subjectId/students/:studentId/homework",
  async (req, res) => {
...
router.post("/", studentIsLoggedIn, async (req, res) => {
  //I need to collect everything I will need for this in my req.body
  const { assignment, description, due_date, priority, subjectId, teacherId } =
    req.body;
  ...
  try {
   ...
   `);
    ...
    const InsertedHomeworkQuery = await db(
      `SELECT * FROM students_subjects_homeworks WHERE homeworkID = LAST_INSERT_ID();`
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Homework assignment updated successfully" });
  } catch (error) {
