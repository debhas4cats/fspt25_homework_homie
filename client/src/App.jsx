...
function App() {
  ...
  const [userData, setUserData] = useState(() => {
    //userData is the current state value. setUserData is a function used to update the state
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await axios.get("/api/student", {
        // GET request to receive data of one student
        headers: {
  ...
  return (
    <Router>
      {/* Router component is used to wrap all our routes */}
      <div>
        <Routes>
          {/* Routes component holds all our defined routes */}
          ...
          <Route path="/calendar" element={<ClickableDate inline />} />{" "}
          {/* Route for ClickableDate */}
        </Routes>
      </div>
    </Router>
  );
